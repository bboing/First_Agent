...
< 사용 방법 >
# env 파일 프로젝트 루트에 넣은 후 돌리기
# 이력 확인용으로 ipynb 파일도 제공 / py와 ipynb는 거의 동일한 소스코드
BASE_DIR = os.path.dirname(os.path.abspath(__file__)) <- 이 부분만 다름 (py)
BASE_DIR = os.getcwd()  # 현재 워킹 디렉토리 <- 이 부분만 다름 (ipynb)

< 향후 논의해야 할 agenda >
# Table Chunking Range
    ㄴ 최소 Chunk 보다 작은 경우, 최소 Chunk 유지
만약 최소 Chunk 보다 큰 경우, Table에서 병합된 부분까지 의미 단위로 쪼개는 방식 
(-> 김태균 과장님 의견)
    -> 개인 의견 : 몇 Depth 까지 고려할 것인가? 1 Depth에서 엄청 길게 나오면?
...
BASE_DIR = os.path.dirname(os.path.abspath(__file__)) # 현재 워킹 디렉토리
        ...
        for file in files:
            ...
            if file_ext == '.pdf':
                ...
                try:
                    ...
                    print(f"파일 처리 중 에러 발생: {file_path}, 에러: {str(e)}")
                    continue
            else:
                print(f"처리할 수 없는 파일 형식: {file_path}")
...
def analyze_documents_combined(file_path):
    ...
    # 25.07.17 timestamp 제거
    output_file = os.path.join(BASE_DIR, "result", "md", f"{filename_wo_ext}.md")
    table_output_file = os.path.join(BASE_DIR, "result", "tables", f"tables_{filename_wo_ext}.txt")
        ...
        with open(table_output_file, "w", encoding="utf-8") as f:
            ...
            def replace_table(match, merged_indices=None):
                ...
                if merged_indices and table_idx in merged_indices and table_idx not in [t["original_index"] for t in table_replacements]:
                    print(f"Removing <table> {table_idx} (merged into another table)")
                    return ""
                
                for table_info in table_replacements:
                    # 인덱스가 일치하는 것만 교체할 것임.
                    if table_info["original_index"] == table_idx:
                        print(f"Replacing <table> {table_idx} with {table_info['key']}")
                        # table 태그를 마크다운으로 바꾼다.
                        return table_info["markdown"]
                print(f"No replacement found for <table> {table_idx}, using original")
    ...
    try:
        ...
        print(f"Table data saved to {table_output_file}")
...
def extract_chunks_from_text(text):
    ...
    for line in lines:
        ...
        if title_match:
            ...
            print('===single title===', single_title)
        ...
        while i < len(tables_data): # merged_tables로 변경하여 동적 갱신 반영
            ...
            while i + 1 < len(tables_data):
                ...
                if idx == 5 or idx == 10 or idx == 11 or idx == 16:
                    if i < 7:
                        print(f'===cur_nxt_tb={idx}행의-{i}번째 테이블==')
                        print('===현재 테이블===\n', current_table)
                        print('=======')
                        print('===다음 테이블===\n', next_table)
                        print('=======')
                        ...
                        print(content)
                        print(content.find(current_table["markdown"]))
                        print(content.find(next_table["markdown"]))
                        print('========')
                ...
                if current_table_end == -1:
                    ...
                    print(f'==마크다운 엑셀 데이터 {idx} 행의 테이블 번호 {i} 번째 찾았습니다.')
                ...
                print(f'===인접했습니다!!=== Between {current_table["key"]} and {next_table["key"]}')

                # 여기서 나오는 반환값은 관리하는 테이블 딕셔너리 (갱신된 상태) 또는 None
                merged_result = merge_tables(current_table, next_table)
                if merged_result:
                    print(f'=====if merged_result >> {idx}행의-{i}번째 테이블=======')
...
def divide_large_passage(df, md_file_name, chunk_size=2000):
    ...
    result_df = pd.DataFrame(new_rows, columns=df.columns)
     # 25.07.17 파일 이름 적용 수정
    result_df.to_excel(f"{BASE_DIR}/result/excel/{md_file_name}_Part5_Output_{timestamp}.xlsx")
...
def add_depth_label(row, is_split=False, is_second_split=False):
 ...
 # 25.07.17 구조 변경
 # md 파일 처리는 한꺼번에
 # excel 처리는 한 개씩. 개별 결과 따로 출력되도록 변경
 # 각 중간결과마다 함수의 인자값으로 md 파일명을 넘겨주어, 개별 파일로 떨어지게 변경
if __name__ == "__main__":
    try:
        print('=[PART 1]==PDF를 마크다운으로 변환하는 작업을 시작합니다.=====')
        results, md_files = process_files_in_directory(DATA_DIR)
        if not results:
        ...
        for md_file, result in zip(md_files, results):
            ...
            print(f"처리된 파일: {result['file_path']}")
            all_tables_data.extend(result['tables_data'])

            md_path = Path(md_file)
            md_file_name = md_path.stem
            print(f"MD 파일 이름: {md_file_name}")
            
            print('===processing md file===', md_file)
            df = batch_process_directory(md_file, md_file_name)

            print(f"표 데이터 개수: {len(all_tables_data)}")
            print('=Part 2 테이블 관리 딕셔너리 출력=\n', all_tables_data)

            print('=[PART 3]===추가 병합 작업을 시작합니다.====')
            df, all_tables_data = merge_adjacent_table(df, all_tables_data, md_file_name)

            print('=[PART 4]===표 데이터 Description 작업을 시작합니다.====')
            ...
            print('=[PART 5]===chunking size에 맞게 분할하는 작업을 시작합니다.====')
            df = divide_large_passage(df, md_file_name, chunk_size=2000)
            print('===최종 Chunking 작업이 완료되었습니다. 엑셀 파일을 확인해주세요.=====')
